...
class ScanResult extends StatefulWidget {
  ...
  ScanResult({this.scan_result, this.preferredStore});
...
class _ScanResultState extends State<ScanResult> {
  ...
  Widget build(BuildContext context) {
    ...
    return widget.scan_result == ''
        ? widget.preferredStore == null
            ? Text(
                'No store was chosen, click  on upper left icon to set',
                style: TextStyle(color: Colors.red),
              )
            : EmptyScan(
                storeName: widget.preferredStore.name,
                storeZipcode: widget.preferredStore.zipCode,
              )
        : StreamBuilder<QuerySnapshot>(
            stream: DatabaseService().getStoreItemStream(
                widget.preferredStore.id, widget.scan_result),
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    String salesInfo = 'On Sales : ${yesOrNo(data['onSale'])}';
                    ...
                    return StreamBuilder<DocumentSnapshot>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          String userUpdateRank = DatabaseService(uid: user.uid)
                            ...
                            children: [
                                    ...
                                    SizedBox(
                                      ...
                                      child: RaisedButton(
                                        ...
                                        onPressed: () => _showUpdateForm(
                                            widget.preferredStore.id, itemId),
                                      ),
                                    ),
                                  ],
                                ),
                              ...
                              StreamBuilder(
                                  stream: DatabaseService(uid: user.uid)
                                      .getItemStream(widget.scan_result),
                                  builder: (context, snapshot) {
                                    if (snapshot.hasError) {
                                      return Text('Something went wrong');
                                    }
